...
static void	check_map_chars(char *str)
{
	int	i;
	...
	while (str[i])
	{
		if (str[i] != '1' && str[i] != '0' && str[i] != 'N'
			&& str[i] != 'S' && str[i] != 'E' && str[i] != 'W'
			&& str[i] != ' ' && str[i] != '\n')
			exit_error(ERROR_INVALID_MAP_CHAR);
		i++;
	}
}
...
static void	parse_config_line(char *line, t_map *map)
{
	...
	if (!ft_strcmp(identifier, "F") || !ft_strcmp(identifier, "C"))
		value = extract_value_no_quotes(line, &pos);
	else
		value = extract_value_with_quotes(line, &pos);
	if (!value)
	{
		free(identifier);
		exit_error(ERROR_LINE_FORMAT);
	}
	check_no_extra_args(line, pos, identifier);
...
void	parse_line(char *line, t_map *map, int map_started)
{
	...
	else
	{
		...
		check_map_chars(line);
		map->map = ft_realloc_map(map->map,
				count_map_lines(map->map), line);
	}
}
