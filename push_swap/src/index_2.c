...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/06/30 10:46:12 by mchrispe          #+#    #+#             */
/*   Updated: 2025/06/30 10:46:14 by mchrispe         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	process_chunk(t_pile *pile, int chunk_min, int chunk_max)
{
	...
	{
		if (pile->lst_a->content >= chunk_min
			&& pile->lst_a->content <= chunk_max)
		{
			push_b(pile);
			if (pile->lst_b->content < (chunk_min + chunk_max) / 2)
			{
				if (pile->lst_a && (pile->lst_a->content < chunk_min
						|| pile->lst_a->content > chunk_max))
				{
					rotate_a_and_b(pile);
					continue ;
