...
// crée une structure t_cmd qui comporte chaque commande (pour les pipes pratique)
...
int attached_outfile(char ***cmds, t_cmd *cmd,
		int *j, t_out_params prm)
{
	...
	if (!create_outfile(cmd->outfile, cmd->append, prm.prm.p,
			prm.prm.nb_cmds))
	{
